...
#SBATCH --output=logs/run4eval_array_%A_%a.out
#SBATCH --error=logs/run4eval_array_%A_%a.err
...
# I/O
# Input = fichier merged produit par sbatch_merge_run4.sh
INPUT=${1:-"${WORKDIR}/data/output/run4_merged_fallback.parquet"}
OUTPUT_BASE=${WORKDIR}/data/output/run4eval
TEXT_COL=text
...
if [[ "${SLURM_ARRAY_TASK_ID:-}" == "0" ]]; then
    sbatch --dependency=afterok:${SLURM_ARRAY_JOB_ID} \
        "${WORKDIR}/sbatch_merge_run4eval.sh" "${SLURM_ARRAY_JOB_ID}"
    echo "[chain] Submitted sbatch_merge_run4eval.sh (dependency: afterok:${SLURM_ARRAY_JOB_ID})"
fi
...
python scripts/run4eval_pipeline.py \
  --input             "$INPUT" \
