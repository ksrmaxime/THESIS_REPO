#!/bin/bash
#SBATCH --job-name=run1_array
#SBATCH --partition=gpu
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=48G
#SBATCH --time=24:00:00
#SBATCH --output=logs/run1_array_%A_%a.out
#SBATCH --error=logs/run1_array_%A_%a.err
#SBATCH --mail-user=maxime.kaiser@unil.ch
#SBATCH --mail-type=END,FAIL
#SBATCH --array=0-7   # 8 GPUs en parallèle → ~6 250 articles chacun pour 50 000 articles
...
# I/O
INPUT=${WORKDIR}/data/input/swissdox_articles_lead.csv
OUTPUT_BASE=${WORKDIR}/data/output/run1
TEXT_COL=text
