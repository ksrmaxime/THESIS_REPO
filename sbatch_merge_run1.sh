#!/bin/bash
#SBATCH --job-name=merge_run1
#SBATCH --partition=cpu
#SBATCH --cpus-per-task=2
#SBATCH --mem=16G
#SBATCH --time=00:30:00
#SBATCH --output=logs/merge_run1_%j.out
#SBATCH --error=logs/merge_run1_%j.err
#SBATCH --mail-user=maxime.kaiser@unil.ch
#SBATCH --mail-type=END,FAIL

# Usage:
#   ARRAY_ID=$(sbatch --parsable sbatch_run1_array.sh)
#   sbatch --dependency=afterok:${ARRAY_ID} sbatch_merge_run1.sh ${ARRAY_ID}
#
# Ou en une commande :
#   ARRAY_ID=$(sbatch --parsable sbatch_run1_array.sh) && \
#   sbatch --dependency=afterok:${ARRAY_ID} sbatch_merge_run1.sh ${ARRAY_ID}
...
if [ -z "$ARRAY_JOB_ID" ]; then
    echo "[ERROR] Passer le ARRAY_JOB_ID en argument: sbatch sbatch_merge_run1.sh <ARRAY_JOB_ID>"
    exit 1
fi
...
MERGED_CSV="${OUTDIR}/run1_merged_job${ARRAY_JOB_ID}.csv"

echo "=== MERGE run1 array job ${ARRAY_JOB_ID} ==="
echo "DATE=$(date -Is)"
...
for col in ["SWISS_CONTEXT", "CRITICISM"]:
...
# =============================================================================
# ARCHIVE — merged result + prompts + sbatch under one run folder
# =============================================================================

WORKDIR=/work/FAC/FDCA/IDHEAP/mhinterl/parp/THESIS_REPO

RUN_DIR="${WORKDIR}/data/output/run1_merged_job${ARRAY_JOB_ID}"
mkdir -p "$RUN_DIR"
...
cp "sbatch_run1_array.sh"       "${RUN_DIR}/sbatch_array_used.sh" || true
