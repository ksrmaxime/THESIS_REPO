...
>> PRE-SCREEN — DO THIS FIRST, BEFORE ANY YES/NO RULE:
   ...
   SOURCE/TARGET CONFUSION CHECK: Before finalising YES, ask: is the Swiss public administration entity I identified as TARGET actually the ACTOR making the critical statements? If a Federal Councillor is quoted criticising private companies, social media platforms, foreign actors, or the healthcare market — that Federal Councillor is the SOURCE, not the TARGET. The TARGET must be a different entity whose actions are being condemned. If you cannot find a SOURCE that is distinct from the TARGET, reconsider whether STEP 2 = YES. <<
...
Answer YES if ANY of the following is present in the article:
  ...
  d) One Swiss admin entity explicitly challenges, opposes, or contests another's proposal, decision, or conduct — including a department opposing another department's proposed policy change. Example: a department's secretariat "vivement critiqué" or "strongly criticised" another department's new travel or mobility rules = YES (SOURCE_TYPE = Another Federal Department or Public Administration Entity). Do not require strong language: even a formal objection or reserved opinion counts.
...
Answer NO if ALL YES checks above returned nothing, AND one of the following applies:
  ...
  g) The article reports neutral, factual information about a government decision, financial planning, or announcement without any actor explicitly condemning a specific entity's past or current conduct. This includes: articles reporting expected future budget deficits, forthcoming spending packages, financial forecasts, or planned structural measures, where the reporting is analytical and informational and no specific actor says the entity's conduct was wrong, inadequate, or harmful. The mere existence of a financial problem does not imply criticism of the entity managing it. Example: an article reporting that the Finanzdepartement will need another savings package in 2029 because of structural deficits, without any actor condemning that management, = NO. This also applies when a minister or department ITSELF announces higher-than-expected costs, delays, or budget adjustments: if the reporting official is informing about the situation without any external actor (journalist, politician, citizen) condemning the administration's past decisions, = NO. Example: a new defence minister announcing that the F-35 purchase may cost 1.3 billion francs more than planned, reported factually without any actor condemning the VBS's past management of the procurement = NO.
  CRITICAL DISTINCTION — SOURCE vs TARGET: Do not confuse an article where a Swiss official is the SOURCE of criticism (advocating for new policies, calling for stricter regulation of private entities, warning about risks) with an article where that official is the TARGET of criticism. When a Federal Councillor speaks about future policy goals or advocates for regulatory change directed at private actors (tech companies, drug manufacturers, etc.) and NO actor in the article criticises that Federal Councillor's own conduct → NO per (b). Example: an article reporting that a Bundesrätin calls for better child protection on social media and is working on a new law, with no actor criticising her past decisions or management, = NO.
...
Choose the single best category from this list:
  Federal Department         — one of the 7 Federal Departments headed by a Federal Councillor (EDA, EDI, EJPD, VBS/DDPS, EFD, UVEK, WBF) or the Federal Chancellery (BK). Use this when the VBS, EFD, EJPD, EDA, etc. is the entity criticised, even if a named official is also mentioned. Also use when the Bundesrat/Federal Council as a collective body is criticised. Also use when criticism targets a broad POLICY SYSTEM, FINANCING SCHEME, or REGULATORY FRAMEWORK that is the statutory responsibility of a Federal Department, even if no specific department is named in the article (e.g. criticism of hospital financing rules "Spitalfinanzierung" established by federal law = Federal Department; criticism of the procurement policy of the VBS = Federal Department).
  ...
  Political Figure           — a named elected or appointed political individual (Federal Councillor criticised in a personal/political capacity rather than as department head, member of parliament, cantonal councillor, mayor, etc.). Examples: a Federal Councillor criticised for their personal statements, communication style, or individual political positions → Political Figure; an MP criticised for their vote or behaviour → Political Figure. DISAMBIGUATION from Federal Department: ask whether the criticism targets the INSTITUTION's policy/decision or the INDIVIDUAL's personal conduct/positions. If it is primarily about the person, use Political Figure. STRONG SIGNAL FOR POLITICAL FIGURE: if critics use the Federal Councillor's name as the primary grammatical subject of evaluative language about their personal leadership, management culture, or individual character (e.g., "Amherds Amtsführung", "ihre Amigo- und Pleitenkultur", "sie hat … diffamieren lassen") → use POLITICAL FIGURE even if the department name also appears in the text. STRONG SIGNAL FOR FEDERAL DEPARTMENT: if the department acronym or full name (VBS, EFD, DFAE, EDI, etc.) is the primary subject of criticism and the individual is mentioned only as the head of that institution.
