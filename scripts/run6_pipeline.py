...
from src.run6_prompts import build_system_prompt, build_user_prompt
from src.run6_config import build_mask

OUTPUT_COLS = ["criticism_target"]

VALID_TARGETS = {"Person", "Policy", "Both", "Unclear"}


def parse_output(raw: str) -> dict:
    empty = {"criticism_target": pd.NA}
    if not raw:
        return empty
    m = re.search(r"TARGET:\s*(\w+)", raw, flags=re.IGNORECASE)
    if m:
        val = m.group(1).strip().capitalize()
        if val in VALID_TARGETS:
            return {"criticism_target": val}
        # Try case-insensitive match
        for v in VALID_TARGETS:
            if v.lower() == val.lower():
                return {"criticism_target": v}
    return empty
...
def main() -> int:
    ...
    # Compute expected chunk boundaries first so we can validate any checkpoint.
    ...
    if checkpoint_path and Path(checkpoint_path).exists():
        ...
        # Reject stale checkpoints whose index set doesn't match current parameters.
    ...
    # Group by keyword so rows in a batch share the same system prompt
    out = df.copy()
    keywords = sorted(df.loc[mask, "keyword"].unique()) if "keyword" in df.columns else [None]

    for keyword in keywords:
        if keyword is not None:
            group_mask = mask & (df["keyword"] == keyword)
        else:
            group_mask = mask
        ...
        system_prompt = build_system_prompt(keyword or "")
    ...
    filled = int(out["criticism_target"].notna().sum())
    print(f"Saved: {parquet_path} | {len(out):,} rows total ({filled:,} with criticism_target)")
