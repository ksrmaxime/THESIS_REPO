...
from src.run5eval_prompts import SYSTEM_PROMPT, build_user_prompt
from src.run5eval_config import build_mask

OUTPUT_COLS = ["run5_valid"]


def parse_output(raw: str) -> dict:
    if not raw:
        return {"run5_valid": pd.NA}
    answer = raw.strip().upper()
    if answer.startswith("YES"):
        return {"run5_valid": "YES"}
    elif answer.startswith("NO"):
        return {"run5_valid": "NO"}
    return {"run5_valid": pd.NA}
...
def main() -> int:
    ...
    ap.add_argument("--input",        required=True,
                    help="Standardized run5 output file (.parquet or .csv)")
    ...
    print(f"[pipeline] {len(df):,} rows in chunk → {valid_count:,} with source_category/reason to validate")
    ...
    yes_count = int((out["run5_valid"] == "YES").sum())
    no_count  = int((out["run5_valid"] == "NO").sum())
    print(f"Saved: {parquet_path} | {len(out):,} rows total (run5_valid: {yes_count:,} YES / {no_count:,} NO)")
